...
Finally, we created the skills table and exported it:
...
The attached SQL script was used to generate the tables and load the data. In loading the data, we found that two main tables still contained an excessive number of rogue commas and invalid characters, which R handled correctly in dataframes but MySQL could not import to tables.

To resolve: 1. All files were converted to tab-delimited before loading, to handle delimiter issues in text strings like job description or skill description

2.  The two main tables also required multiple iterations of removing invalid characters

One error required us to denormalize the job posting skills table as a workaround due to time constraints: joins to the planned lookup table "skills_master" containing skill descriptions failed in testing and we found a large number of silent loading errors ("show warnings" command) that could not be resolved in time. Although the resulting job_posting_skills_w_desc table was not normalized, the functionality of the data was maintained.
